...
describe("game engine", () => {
  ...
  it("keeps core state inside valid bounds during a full automated game", () => {
    ...
    // 超时给到 30 秒：这一局要跑两万多步才到 gameOver，逐步断言的开销都在这里。
  }, 30_000);
  ...
  /*
    ...
    补一条实测（2026-08-10，补 N 档装备时顺带量的）：这颗种子现在跑得到 gameOver
    了，改前 17433 步、改后 24562 步，上面那条不变量用例也确实一路跑到了结束。
    也就是说放开的前置条件已经满足，但「自动对局能通关」是一条关于数值的承诺，
    放开它等于把每次内容改动都绑上这颗种子——所以留给标定成长曲线时一并决定，
    这里只把事实记下来，不擅自 unskip。
  */
